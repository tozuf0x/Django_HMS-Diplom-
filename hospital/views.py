...
def discharge_patient_view(request,pk):
    patient=models.Patient.objects.get(id=pk)
    days=(date.today()-patient.admitDate) #2 дн, 0:00:00
    ...
    patientDict={
        ...
        'day':d,
        'assignedDoctorName':(assignedDoctor[0].first_name + " " + assignedDoctor[0].last_name),
    }
    if request.method == 'POST':
        feeDict ={
        ...
        pDD.assignedDoctorName=assignedDoctor[0].first_name + " " + assignedDoctor[0].last_name
...
def admin_view_appointment_view(request):
    ...
    if appointmentForm.is_valid():
        ...
        (id=request.POST.get('doctorId')).first_name  + " " + assignedDoctor[0].last_name
    # assignedDoctorName = assignedDoctor[0].first_name + " " + assignedDoctor[0].last_name
...
def doctor_dashboard_view(request):
    #for three cards
    patientcount=models.Patient.objects.all().\
        filter(status=True,assignedDoctorId=request.user.id).count()
    appointmentcount=models.Appointment.objects.all().\
        filter(status=True,doctorId=request.user.id).count()
    patientdischarged=models.PatientDischargeDetails.\
        objects.all().distinct().filter(assignedDoctorName=request.user.first_name).count()
...
def doctor_patient_view(request):
    mydict={
    'doctor':models.Doctor.objects.get
    (user_id=request.user.id), #картинка профиля
    }
...
def update_patient_view(request,pk):
    ...
    if request.method=='POST':
        ...
        if userForm.is_valid() and patientForm.is_valid():
            ...
            patient.assignedDoctorId=request.POST.get('assignedDoctorId')
            patient.save()
            return redirect('doctor-update-patient')
...
def patient_book_appointment_view(request):
    ...
    if request.method=='POST':
        ...
        if appointmentForm.is_valid():
            ...
            if doctor.department == 'Кардиолог':
                if 'heart' in desc:
                    pass
                else:
                    print('else')
                    message="Пожалуйста, выберите врача соответствующего заболеванию!"
            ...
            if doctor.department == 'Дерматолог':
                if 'кожа' in desc:
                    pass
                else:
                    print('else')
...
def patient_discharge_view(request):
    ...
    if dischargeDetails:
        ...
        'admitDate':patient.admitDate,
        'releaseDate':dischargeDetails[0].releaseDate,
        'daySpent':dischargeDetails[0].daySpent + 1,
        'medicineCost':dischargeDetails[0].medicineCost,
        # 'roomCharge':dischargeDetails[0].roomCharge,
        'doctorFee':dischargeDetails[0].doctorFee,
        'OtherCharge':dischargeDetails[0].OtherCharge,
...
def contactus_view(request):
    ...
    if request.method == 'POST':
        ...
        if sub.is_valid():
            ...
            name=sub.cleaned_data['Имя']
            message = sub.cleaned_data['Сообщение']
